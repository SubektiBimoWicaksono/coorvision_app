...
from app.routes.admin_routes import tracking_message_queue
# ====================================================================
...
violation_message_queue = queue.Queue(maxsize=100)
...
# ====================================================================
# Fungsi Save CSV untuk Pelanggaran
# ====================================================================
VIOLATION_CSV_LOG_DIR = 'violation_logs'
os.makedirs(VIOLATION_CSV_LOG_DIR, exist_ok=True)

def log_violation_to_csv(timestamp, camera_name, personnel_id, personnel_name, violation_type, confidence, image_path):
    file_path = os.path.join(VIOLATION_CSV_LOG_DIR, f"violation_log_{datetime.now().strftime('%Y-%m-%d')}.csv")
    file_exists = os.path.exists(file_path)
    with open(file_path, mode='a', newline='') as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow([
                "Timestamp", "Camera", "Personnel ID", "Personnel Name",
                "Violation Type", "Confidence", "Image Path"
            ])
        writer.writerow([
            timestamp, camera_name, personnel_id, personnel_name,
            violation_type, f"{confidence:.2f}", image_path
        ])
# ====================================================================
...
def release_camera_instance(camera_source):
    # Lepaskan baik int maupun str
    for key in [camera_source, str(camera_source), int(camera_source) if str(camera_source).isdigit() else None]:
        if key in _camera_instance_cache:
            cap = _camera_instance_cache[key]
            if cap.isOpened():
                cap.release()
            del _camera_instance_cache[key]
            print(f"Camera instance for source '{key}' released.")
...
def save_object_detection(camera, detected_class, confidence, image_path=None, app_obj=None, personnel_id=None):
    try:
        ...
        with app_obj.app_context():
            ...
            notif = {
                "type": "success",
                "message": f"Pelanggaran '{db_detected_class}' terdeteksi dan disimpan (Confidence: {confidence:.2f})"
            }
            tracking_message_queue.put_nowait(json.dumps(notif))
...
def detect_yolo_and_face_onnx(cam=None, static_folder=None, yolo_models_path=None, app_obj=None):
    ...
    try:
        with app_obj.app_context():
            frame_count = 0
            while True:
                flush_camera_buffer(cap, flush_count=5)
                ret, frame = cap.read()
                if not ret or frame is None or frame.size == 0:
                    time.sleep(0.05)
                    continue
                if len(frame.shape) != 3 or frame.shape[0] < 100 or frame.shape[1] < 100:
                    print("Warning: Frame shape not valid, skipping.")
                    continue
                ...
                # (Logika deteksi wajah Anda tetap sama)
                ...
                # Gambar semua box pelanggaran dari YOLO
                for i in range(len(yolo_boxes)):
                    class_name = yolo_class_names[yolo_clss[i]]
