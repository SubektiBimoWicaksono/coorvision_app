from flask import Blueprint, render_template, request, redirect, url_for, flash, jsonify, current_app, send_file, Response, stream_with_context
...
# --- Dashboard Route ---
# Queue global untuk notifikasi tracking (letakkan di atas)
tracking_message_queue = queue.Queue(maxsize=100)

@bp.route('/dashboard')
@login_required
@admin_required
def dashboard():
    # Ambil objek company berdasarkan user yang login
    ...
    # Pelanggaran summary (total/no_tie)
    today = datetime.now().date()
    filter_date_str = request.args.get('filter_date', today.strftime('%Y-%m-%d'))
    try:
    ...
    tracking_cams = Camera_Settings.query.filter_by(company_obj=company, role_camera=Camera_Settings.ROLE_TRACKING).all()
    tracking_cam_ids = [cam.id for cam in tracking_cams]
    ...
    if tracking_cam_ids:
        tracking_query = (
            ...
            .filter(
                Tracking.camera_id.in_(tracking_cam_ids),
                func.date(Tracking.timestamp) == filter_date
            )
            .group_by(Tracking.detected_class)
        )
        total = 0
        for row in tracking_query:
            total += row.count
            if row.detected_class in ['no-tie', 'Tidak Memakai Dasi']:
                pelanggaran_summary['no_tie'] += row.count
        pelanggaran_summary['total'] = total

    # Untuk grafik: pelanggaran per hari per personel
    ...
    filter_day = request.args.get('filter_day')  # 0=Senin, dst

    days_back = 7
    date_list = [(today - timedelta(days=i)) for i in range(days_back-1, -1, -1)]

    violation_per_day = {}
    for d in date_list:
        query = Tracking.query.filter(
            Tracking.camera_id.in_(tracking_cam_ids),
            func.date(Tracking.timestamp) == d
        )
        if filter_personnel_id and filter_personnel_id.isdigit():
            query = query.filter(Tracking.personnel_id == int(filter_personnel_id))
        if filter_day is not None and filter_day != '':
            if d.weekday() != int(filter_day):
                violation_per_day[d.strftime('%Y-%m-%d')] = 0
                continue
        violation_per_day[d.strftime('%Y-%m-%d')] = query.count()

    # Pastikan data grafik tidak None
    violation_chart_labels = [d.strftime('%Y-%m-%d') for d in date_list]
    violation_chart_data = [violation_per_day.get(d.strftime('%Y-%m-%d'), 0) for d in date_list]
    
    division_count = Divisions.query.filter_by(company_obj=company).count()
    employee_count = Personnels.query.filter_by(company_obj=company).count()
    
    # Filter bulan & tahun dari request
    filter_month = request.args.get('filter_month', today.strftime('%Y-%m'))
    try:
        filter_month_dt = datetime.strptime(filter_month, '%Y-%m')
        month_num = filter_month_dt.month
        year_num = filter_month_dt.year
    except ValueError:
        month_num = today.month
        year_num = today.year
        filter_month = today.strftime('%Y-%m')

    # Query jumlah pelanggaran per pegawai di bulan yang dipilih
    employee_violations = (
        db.session.query(
            Personnels.id,
            Personnels.name,
            func.count(Tracking.id).label('violation_count')
        )
        .outerjoin(Tracking, db.and_(
            Tracking.personnel_id == Personnels.id,
            extract('month', Tracking.timestamp) == month_num,
            extract('year', Tracking.timestamp) == year_num,
            Tracking.camera_id.in_(tracking_cam_ids)
        ))
        .filter(Personnels.company_obj == company)
        .group_by(Personnels.id, Personnels.name)
...
def export_tracking_report_excel():
    ...
    for idx, entry in enumerate(tracking_query, start=1):
        # Mapping hanya dua pelanggaran
        if entry.detected_class in ['no-tie', 'Tidak memakai dasi']:
...
def delete_tracking(tracking_id):
    ...
    image_path = tracking_entry.image_path
    try:
        # Hapus file gambar jika ada dan path valid
...
def tracking_events():
    def event_stream():
        ...
        while True:
            try:
                msg = tracking_message_queue.get(timeout=60)
                if msg and msg not in announced:
                    yield f"data: {msg}\n\n"
                    announced.add(msg)
            except queue.Empty:
                yield ": keepalive\n\n"
